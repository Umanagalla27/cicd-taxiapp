        ...
        stage("Fetch Infrastructure Details") {
            steps {
                script {
                    ...
                    // Find only the FIRST S3 bucket by name pattern or tags (using | [0] to ensure only one value)
        ...
        stage('SonarQube Analysis') {
            steps {
                script {
                    ...
                    mvn verify org.sonarsource.scanner.maven:sonar-maven-plugin:sonar -Dsonar.projectKey=taxi-app1234_taxi -Dsonar.organization=taxi-app1234 -Dsonar.host.url=https://sonarcloud.io -Dsonar.token=$SONAR_TOKEN
                }
            }
        }
